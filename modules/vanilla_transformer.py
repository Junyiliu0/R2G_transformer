...
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def __init__(self, h, d_model, m):
        ...
        self.m_k = nn.Parameter(torch.FloatTensor(1, m, h * self.d_k))
        self.m_v = nn.Parameter(torch.FloatTensor(1, m, h * self.d_v))
    ...
    def forward(self, queries, keys, values, attention_mask=None, layer_past=None):
        '''
        Computes
